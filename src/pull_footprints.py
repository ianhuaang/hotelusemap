"""Pull building footprints for Manhattan, Brooklyn, and Queens from NYC Open Data.

Pulls all three boroughs. The pipeline join naturally filters to only
buildings that survived the PLUTO filter (which is CD-scoped for BK/QN).
"""

import json
import ssl
import time
import urllib.error
import urllib.parse
import urllib.request
from datetime import date
from pathlib import Path

import certifi

import sys
sys.path.insert(0, str(Path(__file__).parent.parent))
from config import DATA_RAW, SOCRATA_BASE_URL, BUILDING_FOOTPRINTS_DATASET_ID

# 10k measured fastest for this dataset: ~84 requests at ~5MB each, vs 419
# requests at 2k. Larger pages get slower per byte and spike memory, since
# these rows carry the_geom polygon geometry.
BATCH_SIZE = 10000
COLUMNS = [
    "the_geom", "base_bbl", "mappluto_bbl", "bin", "height_roof",
    "construction_year", "feature_code", "doitt_id",
]


def pull_footprints() -> Path:
    ctx = ssl.create_default_context(cafile=certifi.where())
    today = date.today().strftime("%Y%m%d")
    outfile = DATA_RAW / f"footprints_{today}.json"

    if outfile.exists():
        print(f"Already exists: {outfile}")
        return outfile

    DATA_RAW.mkdir(parents=True, exist_ok=True)

    select = ",".join(COLUMNS)
    where = "base_bbl like '1%' OR base_bbl like '3%' OR base_bbl like '4%'"
    all_rows = []
    offset = 0

    while True:
        params = urllib.parse.urlencode({
            "$select": select,
            "$where": where,
            "$limit": BATCH_SIZE,
            "$offset": offset,
            "$order": "base_bbl",
        })
        url = f"{SOCRATA_BASE_URL}/{BUILDING_FOOTPRINTS_DATASET_ID}.json?{params}"
        req = urllib.request.Request(url, headers={"User-Agent": "nyc-transient-capacity/0.1"})

        batch = None
        for attempt in range(4):
            try:
                with urllib.request.urlopen(req, timeout=180, context=ctx) as resp:
                    batch = json.loads(resp.read())
                break
            except (ConnectionResetError, TimeoutError, urllib.error.URLError) as e:
                if attempt == 3:
                    raise
                wait = 10 * (attempt + 1)
                print(f"  retry {attempt + 1} at offset {offset}: {e} (waiting {wait}s)")
                time.sleep(wait)

        if not batch:
            break

        all_rows.extend(batch)
        print(f"  fetched {len(all_rows)} footprints...")
        offset += BATCH_SIZE
        time.sleep(0.5)  # matches every other pull script

    outfile.write_text(json.dumps(all_rows, indent=2))
    print(f"Saved {len(all_rows)} footprint rows -> {outfile}")
    return outfile


if __name__ == "__main__":
    pull_footprints()
